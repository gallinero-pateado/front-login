...
const DICCIONARIO_ERRORES = {
  ...
  perfil: {
    incomplete_profile:
      "Debe completar todos los campos obligatorios del perfil",
  },
...
const obtenerMensajeError = (categoria, codigoError, errorBackend = null) => {
  ...
  if (
    DICCIONARIO_ERRORES[categoria] &&
    DICCIONARIO_ERRORES[categoria][codigoError]
  ) {
...
const Login = () => {
  ...
  useEffect(() => {
    axios.interceptors.request.use((config) => {
      ...
      config.withCredentials = true; // Incluir credenciales en las solicitudes
      return config;
    });
  }, []);
    ...
    try {
      ...
      if (isEmployeeLogin) {
        ...
        try {
          ...
          navigate(perfil_completado ? "/unificacion" : "/complete_profile");
        } catch (profileError) {
  ...
  return (
    <div
      className={`min-h-screen flex flex-col items-center justify-center ${
        theme === "dark" ? "bg-gray-900" : "bg-[#DAEDF2]"
      ...
      >
        <h2
          className={`text-4xl font-bold mb-8 ${
            theme === "dark" ? "text-[#A3D9D3]" : "text-[#0092BC]"
          } text-center`}
        >
          Iniciar Sesión
        </h2>
        ...
        <div className="flex flex-col items-center mb-4">
          <button
            ...
            className={`${
              theme === "dark"
                ? "bg-[#A3D9D3] hover:bg-[#8ec3c0] text-gray-800"
                : "bg-[#0092BC] hover:bg-[#007a9a] text-white"
            } font-bold py-4 px-8 rounded-lg focus:outline-none focus:shadow-outline transition duration-300 mb-2 ${
          ...
          <button
            ...
            className={`${
              theme === "dark"
                ? "bg-gray-700 hover:bg-gray-600 text-[#A3D9D3]"
                : "bg-[#A3D9D3] hover:bg-[#8ec3c0] text-white"
            } font-bold py-4 px-8 rounded-lg focus:outline-none focus:shadow-outline transition duration-300`}
